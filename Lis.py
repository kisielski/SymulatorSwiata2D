...
class Lis(Zwierze):
	...
	def __init__(self, x, y, swiat):
		...
		self._Organizm__typ = 'L'
		self._Organizm__swiat = swiat
		self._Organizm__img = pygame.image.load(os.path.join("assets", "lis.png"))
	...
	def akcja(self, com):
		...
		if self._Organizm__swiat.czyPustePole(pozycja[0], pozycja[1]):
			...
			if self._Organizm__swiat.getOrganizm(pozycja[0], pozycja[1])._Organizm__sila <= self._Organizm__sila:
				self.kolizja(self._Organizm__swiat.getOrganizm(pozycja[0], pozycja[1]), com)
		self._Organizm__wiek += 1

	def porod(self, x, y):
		self._Organizm__swiat.dodajOrganizm(x, y, Lis(x, y, self._Organizm__swiat))